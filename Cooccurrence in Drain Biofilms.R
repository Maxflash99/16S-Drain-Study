...
setwd(dir = "~/Desktop/16SDrainStudy/core-metrics-results")
...
treatments<-as.vector(unique(dataset$Store))
datasetn<-dataset
...
ASVs<-subset(dataset, get(my_column)==treatments[1])

colnames(ASVs[1:10])
head(final_results)
ggplot(ASVs, aes(x = ASV1, y = ASV10)) +
  geom_point()
...
write.csv(subset(strong_results_taxa, trt=="S5"), "output/moving-pictures-strong-results-taxa-S5.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="S6"), "output/moving-pictures-strong-results-taxa-S6.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="S7"), "output/moving-pictures-strong-results-taxa-S7.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="S8"), "output/moving-pictures-strong-results-taxa-S8.csv", row.names = F)
