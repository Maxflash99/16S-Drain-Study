...
getwd()
###Set your working directory path/to/ANSC516/ANSC516-repo/ANSC516/data/moving-pictures
setwd("~/Desktop/16SDrainStudy/core-metrics-results/")
list.files()
...
meta<-read_q2metadata("Metadata.txt")
str(meta)
...
hist(meta$faithpd, main="Faith phylogenetic diversity", xlab="", breaks=10)
...
ggqqplot(meta$faithpd, title = "Faith PD")
...
aov.pielou_evenness.Store = aov(pielou_evenness ~ Store, data=meta)
#Call for the summary of that ANOVA, which will include P-values
summary(aov.pielou_evenness.Store)
...
TukeyHSD(aov.pielou_evenness.Store)

# We clearly see that the pielou_evenness between hands and gut are different. 
# When we plot the data, we see that pielou_evenness decreases in the gut 
# compared to palms.

levels(meta$Store)
#Re-order the groups because the default is alphabetical order
meta$Store.ord = factor(meta$Store, c("S1", "S2", "S3", "S4", "S5", "S6", "S7", "S8"))
levels(meta$Store.ord)

#Plot
boxplot(pielou_evenness ~ Store, data=meta, ylab="pielou_evenness")

evenness_boxplot <- ggplot(meta, aes(Week.ord, pielou_evenness)) + 
  geom_boxplot() + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
...
evenness_summary <- meta %>% # the names of the new data frame and the data frame to be summarised
  group_by(Week.ord) %>%   # the grouping variable
...
evenness_se <- ggplot(evenness_summary, aes(Week.ord, mean_evenness, fill = Week.ord)) + 
  geom_col() + 
...
kruskal.test(faith_pd ~ Week.ord, data=meta)
...
pairwise.wilcox.test(meta$observed_features, meta$Week.ord, p.adjust.method="BH")

# Like evenness, we see that pd also increases with age.

#Plot
boxplot(pielou_evenness ~ Week.ord, data=meta, ylab="Faith phylogenetic diversity")

# or with ggplot2

pielou_evenness_boxplot <- ggplot(meta, aes(Week.ord, pielou_evenness)) + 
  geom_boxplot(aes(color = Week.ord)) + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  theme(legend.title = element_blank()) +
  labs(y="Faith Phylogenetic Diversity", x = "") 
ggsave("output/pd.png", pielou_evenness_boxplot, height = 3, width = 3)
...
# distributed metric pielou_evenness. However, this time, we change the 
...
gaussian.faith.time = glm(pielou_evenness ~ Week, data=meta, family="gaussian")
plot(gaussian.faith.time, which=c(1,2))

# Quasipoisson (log) distribution
qp.faith.time = glm(pielou_evenness ~ days.since.experiment.start, data=meta, family="quasipoisson")
...
# Plotting this we see that, indeed, there is a trend toward correlation between pielou_evenness and days.since.experiment.start.

#Plot
plot(log(pielou_evenness) ~ Week, data=meta, ylab="ln(Faith Phylo. Diversity)")
plot(log(pielou_evenness) ~ Week, data=meta, ylab="ln(Faith Phylo. Diversity)") + abline(qp.faith.time)
