...
metadata2 <- read.delim("Metadata.txt", sep = "\t", header = T, quote = "", stringsAsFactors = F)
metadata2[1,]
metadata2[,1]
...
ggplot(bc_meta, aes(x=PC1, y=PC2, color=Store)) +
  geom_point() + #alpha controls transparency and helps when points are overlapping
  theme_q2r() +
  xlab("PC1 (32.27%)") +
  ylab("PC2 (22.28%)") +
  scale_color_manual(values=c("Black", "Blue", "Green", "Gray", "Red", "Yellow", "Orange", "Pink"), name = "body-site")
...
ggplot(Jacc_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  ...
  scale_color_manual(values=body_colors, name = "Store")
...
metadata_sub <- metadata[match(rownames(Jacc_dm),metadata$SampleID),]
rownames(Jacc_dm) == metadata_sub$SampleID ## all these values need to be "TRUE"

PERMANOVA_out <- adonis2(Jacc_dm ~ Week, data = metadata_sub)

write.table(PERMANOVA_out,"output/Store_Adonis_overall.csv",sep=",", row.names = TRUE) 
...
Store_Pair <- pairwise.adonis2(bc_dm ~ Store, data = metadata_sub)
write.table(Store_Pair,"output/Store_Adonis_pairwise.csv",sep=",", row.names = TRUE) 
